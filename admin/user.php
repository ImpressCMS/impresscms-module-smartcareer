...
function edituser($showmenu = false, $userid = 0, $parentid =0)
{
	...
	if (!$userObj->isNew()){
		...
		if ($showmenu) {
			smart_adminMenu(1, _AM_SCAREER_USERS . " > " . _CO_SOBJECT_EDITING);
		}
		...
		if ($showmenu) {
			smart_adminMenu(1, _AM_SCAREER_USERS . " > " . _CO_SOBJECT_CREATINGNEW);
		}
...
switch ($op) {
	...
	case "view" :
		...
		smart_adminMenu(1, _AM_SCAREER_USER_VIEW . ' > ' . $userObj->getVar('name'));

		smart_collapsableBar('userview', $userObj->getVar('name') . $userObj->getEditItemLink(), _AM_SCAREER_USER_VIEW_DSC);
	...
	default:
		...
		smart_adminMenu(1, _AM_SCAREER_USERS);
