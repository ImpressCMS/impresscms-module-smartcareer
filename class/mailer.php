...
class SmartcareerMailer {
	...
	function execute() {
		...
		$smartcareer_config = smart_getModuleConfig('smartcareer');

		$smartcareer_message_handler = xoops_getModuleHandler('message', 'smartcareer');
		...
		$newMessagesObj = $smartcareer_message_handler->getNewMessages();
		if ($newMessagesObj) {
			foreach($newMessagesObj as $newMessageObj) {
				$this->addLog('Message : "' . $newMessageObj->getVar('subject') . '" is new');
				if (!$smartcareer_message_handler->addRecipients($newMessageObj)) {
		...
		//$smartcareer_message_handler->purgeSentMessages();

		// are there any New ecards
		$newEcardsObj = $smartcareer_ecard_handler->getNewEcards();
		if ($newEcardsObj) {
			foreach($newEcardsObj as $newEcardObj) {
				$this->addLog('eCard : "' . $newEcardObj->getVar('subject') . '" is new');
				if (!$smartcareer_ecard_handler->addRecipients($newEcardObj)) {
					$this->addLog('An error occured while adding the recipients');
				} else {
		...
		//$smartcareer_ecard_handler->purgeSentEcards();

		// are there any email to send
		$recipentsObj = $smartcareer_recipient_handler->getReadyRecipients();
		...
		$messagesObj = $smartcareer_message_handler->getTheseMessages($messageidArray);

		// fetch the concerned ecardsObj
		$ecardsObj = $smartcareer_ecard_handler->getTheseEcards($ecardidArray);
		...
		foreach ($recipentsObj as $recipientObj) {
			...
			if (!$xoopsMailer->send(true)) {
				...
				$recipientObj->setVar('status', SMARTCAREER_RECIPIENT_STATUS_SENT);
				$smartcareer_recipient_handler->insert($recipientObj, true);
			}
			$xoopsMailer->reset();
		}
	}
